...
class SocialAuthController @Inject() (val messagesApi: MessagesApi,
  ...
  extends Silhouette[SilhouetteUser, CookieAuthenticator] with Logger {
  ...
  def authenticate(provider: String) = Action.async { implicit request =>
    ...
    (socialProviderRegistry.get[SocialProvider](provider) match {
      ...
      case e: ProviderException =>
        ...
        Redirect(routes.ApplicationController.signIn()).flashing("error" -> Messages("auth.provider.error"))
    }
  }
}