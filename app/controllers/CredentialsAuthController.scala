...
class CredentialsAuthController @Inject() (val messagesApi: MessagesApi,
                                           ...
                                           val clock: Clock) extends Silhouette[SilhouetteUser, CookieAuthenticator] {
      ...
      data => {
        ...
        credentialsProvider.authenticate(credentials).flatMap { loginInfo =>
          ...
          case e: ProviderException =>
            Redirect(routes.ApplicationController.signIn()).flashing("error" -> Messages("auth.invalid.credentials"))
        }
      }
    )
  }
